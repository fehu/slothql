...
import com.github.fehu.slothql.cypher.{ CypherFragment, GraphElem }
...
class CypherSyntaxReadSpec extends CypherSyntaxBaseSpec {
  ...
  "Slothql cypher syntax" should {
    ...
    "support optional `if` guards (defined)" in {
      def condOpt(n: Node): Option[Expr[Boolean]] = Some(n.prop[String]("key") === lit("foobar"))
      test(
    ...
    "require all WITH modifiers to be defined on top of WITH block" in
      illTyped(
        """With(lit(10)) { n =>
           val x: com.github.fehu.slothql.cypher.CypherFragment.Expr[Int] = lit(100)
           With.where(lit(true))
           Match { case a =>
             (a.props, n, x)
           }}""",
