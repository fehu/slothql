package com.github.fehu.slothql.cypher.syntax
...
import com.github.fehu.slothql.cypher.CypherFragment.{ Pattern => P }
import com.github.fehu.slothql.cypher.CypherStatement.Alias
import com.github.fehu.slothql.cypher.{ CypherStatement, GraphElem, syntax, CypherFragment => CF }
...
class CypherSyntaxPatternMacros(val c: blackbox.Context) {
        ...
        case List(CaseDef(pattern, guard0, body)) =>
          ...
          val (rebind0, bindDefsNodeRel) = elems0.collect{
            case (Some(nme), Pattern.Tpe.Node, _) =>
              ...
              val tree = q"val ${TermName(name)} = _root_.com.github.fehu.slothql.cypher.syntax.CypherSyntaxFromMacro.mkNode($nme)"
              (nme, name) -> tree
            case (Some(nme), Pattern.Tpe.Rel(dir), _) =>
              val name = c.freshName(nme)
              val tree = q"val ${TermName(name)} = _root_.com.github.fehu.slothql.cypher.syntax.CypherSyntaxFromMacro.mkRel[$dir]($nme)"
              (nme, name) -> tree
          }.unzip
          val bindDefOptLet = letAliasOpt.map{ nme =>
            val name = c.freshName(nme)
            val tree = q"val ${TermName(name)} = _root_.com.github.fehu.slothql.cypher.syntax.CypherSyntaxFromMacro.mkPath($nme)"
            (nme, name) -> tree
          }
  ...
  private lazy val SyntaxPackageTypeSignature = c.mirror.staticPackage("com.github.fehu.slothql.cypher.syntax").typeSignature
  ...
  object PatternPlus {
    ...
    lazy val SyntaxUnapply_::= = rootMirror.staticModule("com.github.fehu.slothql.cypher.syntax.$colon$colon$eq")
  ...
  object Pattern {
      ...
      val (props01, props02, props03) = args.collect {
        case UnApply(m,  List(lhs, rhs)) if m.symbol == Syntax_UnapplySeqSymbol_:= =>
          ...
          val value = if (rhs.tpe <:< CypherExprType) rhs
                      else q"_root_.com.github.fehu.slothql.cypher.syntax.lit($rhs)"
    ...
    lazy val Syntax_<  = rootMirror.staticModule("com.github.fehu.slothql.cypher.syntax.$less")
    lazy val Syntax_>  = rootMirror.staticModule("com.github.fehu.slothql.cypher.syntax.$greater")
    lazy val Syntax_-  = rootMirror.staticModule("com.github.fehu.slothql.cypher.syntax.$minus")
    lazy val Syntax_:= = rootMirror.staticModule("com.github.fehu.slothql.cypher.syntax.$colon$eq")
    lazy val Syntax_** = rootMirror.staticModule("com.github.fehu.slothql.cypher.syntax.$times$times")
