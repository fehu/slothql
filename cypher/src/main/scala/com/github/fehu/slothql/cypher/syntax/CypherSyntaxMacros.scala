...
class CypherSyntaxMacros(val c: whitebox.Context) {
  ...
  def returnTuple[T: WeakTypeTag]: Tree = {
    ...
    q"""
      new _root_.com.github.fehu.slothql.cypher.syntax.CypherSyntaxReturnTuple[$T] {
        type Out = ${mkTupleType(outTypes)}
        def apply($arg: $T): _root_.com.github.fehu.slothql.cypher.CypherFragment.Return.Return0[Out] =
          _root_.com.github.fehu.slothql.cypher.CypherFragment.Return.Tuple[Out](_root_.scala.List(..$returns))
      }
     """
  }

  private def returnExprTree(tpe: Type, expr: Tree) =
    q"_root_.com.github.fehu.slothql.cypher.CypherFragment.Return.Expr[$tpe]($expr, as = _root_.scala.None)"
