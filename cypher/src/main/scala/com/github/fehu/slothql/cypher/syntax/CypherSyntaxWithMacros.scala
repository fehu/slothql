...
import com.github.fehu.slothql.cypher.{ CypherFragment => CF }
...
class CypherSyntaxWithMacros(override val c: blackbox.Context) extends CypherSyntaxPatternMacros(c) {
  ...
  protected def withImpl[R](query: Tree, wildcard: Boolean, exprs: ExprWithInnerType*): c.Expr[CF.Query.Query0[R]] = {
    ...
    val (argNames, binds, returns) = exprs.zip(argNames0).map {
      case ((tree, tpe), nme) =>
        ...
        val bind = q"val $bindName = _root_.com.github.fehu.slothql.cypher.CypherFragment.Expr.Alias[$tpe]($nme)"
        val ret = q"_root_.com.github.fehu.slothql.cypher.CypherFragment.Return.Expr($tree, _root_.scala.Some($bindName))"
        ((nme, name), bind, ret)
    }.unzip3
    ...
    val return0 = (wildcard, returns) match {
      ...
      case (false, seq)         => q"_root_.com.github.fehu.slothql.cypher.CypherFragment.Return.Tuple(_root_.scala.List(..$seq))"
      case (true, Seq())        => q"_root_.com.github.fehu.slothql.cypher.CypherFragment.Return.Wildcard"
      case (true, seq)          => q"_root_.com.github.fehu.slothql.cypher.CypherFragment.Return.WildcardTuple(_root_.scala.List(..$seq))"
    }
    ...
    val (returnTree, whereTree) = Opt.collect(ops) match {
      ...
      case Right((whereOpt, distinctOpt, skipOpt, limitOpt, orderSeq)) =>
        ...
        lazy val orderBy = orderSeq.map {
          case Opt.OrderByExpr(expr)     => q"_root_.scala.List(($expr, _root_.com.github.fehu.slothql.cypher.CypherFragment.Return.Order.Ascending))"
          case Opt.OrderByOrd(expr, ord) => q"_root_.scala.List(($expr, $ord))"
          case Opt.OrderBySel(expr, ord) => q"_root_.scala.List(($expr, $ord(_root_.com.github.fehu.slothql.cypher.CypherFragment.Return.Order)))"
        ...
        val ret = if (hasOptions) q"_root_.com.github.fehu.slothql.cypher.CypherFragment.Return.Options($return0, $distinct, $orderBy, $skip, $limit)"
                  else return0
    ...
    c.Expr[CF.Query.Query0[R]](
      q"""
        ..$binds
        _root_.com.github.fehu.slothql.cypher.CypherFragment.Query.Clause(
          _root_.com.github.fehu.slothql.cypher.CypherFragment.Clause.With(
            $returnTree,
            $whereTree
          ),
  ...
  private val WithSyntax = c.mirror.staticModule("com.github.fehu.slothql.cypher.syntax.With")
}
