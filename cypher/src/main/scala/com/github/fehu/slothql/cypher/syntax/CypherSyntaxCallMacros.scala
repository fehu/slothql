...
class CypherSyntaxCallMacros(override val c: whitebox.Context) extends CypherSyntaxPatternMacros(c){
  ...
  protected def yieldImpl[R: WeakTypeTag](func: Tree, yieldsT: (Tree, Type)*): Tree = {
      ...
      case Function(yieldsV, body) if yieldsT.nonEmpty =>
        val (rebind, binds, rets) = yieldsT.zip(yieldsV).map {
          case ((nme, tpe), ValDef(_, TermName(alias), _, _)) =>
            ...
            val bind = q"val ${TermName(name)} = _root_.com.github.fehu.slothql.cypher.CypherFragment.Expr.Alias[$tpe]($alias)"
            val ret  =
              q"""
                _root_.com.github.fehu.slothql.cypher.CypherFragment.Return.Expr[$tpe](
                  expr = _root_.com.github.fehu.slothql.cypher.CypherFragment.Expr.Alias.Fixed($nme),
        ...
        val yields = q"_root_.scala.Some(_root_.com.github.fehu.slothql.cypher.CypherFragment.Return.Tuple(_root_.scala.List(..$rets)))"
    ...
    _root_.com.github.fehu.slothql.cypher.CypherFragment.Query.Clause(
      _root_.com.github.fehu.slothql.cypher.CypherFragment.Clause.Call(
        procedure = $procedure,
  ...
  private val CallSymbol = rootMirror.staticModule("com.github.fehu.slothql.cypher.syntax.Call")
}
